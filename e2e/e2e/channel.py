...
class TxChanOpenInit(Cmd[TxChanOpenInitRes]):
    ...
    def args(self) -> List[str]:
        args = [
            self.dst_chain_id, self.src_chain_id, self.connection_id,
...
class TxChanOpenTry(Cmd[TxChanOpenTryRes]):
    ...
    def args(self) -> List[str]:
        args = [
            self.dst_chain_id, self.src_chain_id, self.connection_id,
            self.dst_port_id, self.src_port_id, "-s", self.src_channel_id
        ]
...
class TxChanOpenAck(Cmd[TxChanOpenAckRes]):
    ...
    def args(self) -> List[str]:
        args = [
            self.dst_chain_id, self.src_chain_id, self.connection_id,
            self.dst_port_id, self.src_port_id, "-d", self.dst_channel_id, "-s",
            self.src_channel_id
        ]
...
class TxChanCloseInit(Cmd[TxChanCloseInitRes]):
    ...
    def args(self) -> List[str]:
        args = [
            self.dst_chain_id, self.src_chain_id, self.dst_conn_id,
            self.dst_port_id, self.src_port_id, "-d", self.dst_chan_id, "-s",
            self.src_chan_id
        ]
...
def chan_open_init(c: Config,
                   ...
                   ordering: Optional[Ordering] = None) -> ChannelId:
    cmd = TxChanOpenInit(dst_chain_id=dst,
                         src_chain_id=src,
...
def chan_open_try(c: Config,
                  ...
                  src_chan: ChannelId,
                  ordering: Optional[Ordering] = None) -> ChannelId:
    cmd = TxChanOpenTry(dst_chain_id=dst,
                        src_chain_id=src,
                        connection_id=dst_conn,
                        dst_port_id=dst_port,
                        src_port_id=src_port,
...
def chan_open_ack(
    ...
    cmd = TxChanOpenAck(dst_chain_id=dst,
                        src_chain_id=src,
                        connection_id=dst_conn,
                        dst_port_id=dst_port,
                        src_port_id=src_port,
...
def chan_open_confirm(c: Config, dst: ChainId, src: ChainId,
                      dst_conn: ConnectionId, dst_port: PortId,
                      src_port: PortId, dst_chan: ChannelId,
                      src_chan: ChannelId) -> ChannelId:
    cmd = TxChanOpenConfirm(dst_chain_id=dst,
                            src_chain_id=src,
                            connection_id=dst_conn,
                            dst_port_id=dst_port,
                            src_port_id=src_port,
...
def chan_close_init(c: Config, dst: ChainId, src: ChainId,
                    dst_conn: ConnectionId, dst_port: PortId, src_port: PortId,
                    dst_chan: ChannelId, src_chan: ChannelId) -> ChannelId:
    cmd = TxChanCloseInit(dst_chain_id=dst,
                          src_chain_id=src,
...
def chan_close_confirm(c: Config, dst: ChainId, src: ChainId,
                       dst_conn: ConnectionId, dst_port: PortId,
                       src_port: PortId, dst_chan: ChannelId,
                       src_chan: ChannelId) -> ChannelId:
    cmd = TxChanCloseConfirm(dst_chain_id=dst,
                             src_chain_id=src,
                             dst_conn_id=dst_conn,
                             dst_port_id=dst_port,
                             src_port_id=src_port,
...
def handshake(c: Config, side_a: ChainId, side_b: ChainId, conn_a: ConnectionId,
              conn_b: ConnectionId,
              port_id: PortId) -> Tuple[ChannelId, ChannelId]:
    a_chan_id = chan_open_init(c, dst=side_a, src=side_b, dst_conn=conn_a)

    split()

    b_chan_id = chan_open_try(c,
                              dst=side_b,
                              src=side_a,
                              dst_conn=conn_b,
                              dst_port=port_id,
...
def query_channel_end(c: Config, chain_id: ChainId, port: PortId,
                      chan_id: ChannelId) -> ChannelEnd:
...
def query_channel_ends(c: Config, chain_id: ChainId, port: PortId,
                       chan_id: ChannelId) -> ChannelEnd:
...
def verify_state(c: Config, ibc1: ChainId, ibc0: ChainId,
    ...
    if strategy == 'all':
        ...
        for i in range(20):
            ...
            assert (ibc0_chan_end.state == 'Open'), (ibc0_chan_end,
                                                     "state is not Open")
            assert (ibc1_chan_end.state == 'Open'), (ibc1_chan_end,
                                                     "state is not Open")
...
def passive_channel_start_then_init(c: Config, ibc1: ChainId, ibc0: ChainId,
...
def passive_channel_init_then_start(c: Config, ibc1: ChainId, ibc0: ChainId,
                                    ibc1_conn_id: ConnectionId,
                                    port_id: PortId):
...
def passive_channel_try_then_start(c: Config, ibc1: ChainId, ibc0: ChainId,
                                   ibc1_conn_id: ConnectionId,
                                   ibc0_conn_id: ConnectionId, port_id: PortId):
