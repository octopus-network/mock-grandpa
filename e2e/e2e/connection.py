...
class TxConnInit(Cmd[TxConnInitRes]):
    ...
    def args(self) -> List[str]:
        return [
            self.dst_chain_id, self.src_chain_id, self.dst_client_id,
            self.src_client_id
        ]
...
class TxConnTry(Cmd[TxConnTryRes]):
    ...
    def args(self) -> List[str]:
        return [
            self.dst_chain_id, self.src_chain_id, self.dst_client_id,
            self.src_client_id, "-s", self.src_conn_id
        ]
...
class TxConnAck(Cmd[TxConnAckRes]):
    ...
    def args(self) -> List[str]:
        return [
            self.dst_chain_id, self.src_chain_id, self.dst_client_id,
            self.src_client_id, "-d", self.dst_conn_id, "-s", self.src_conn_id
        ]
...
def conn_init(c: Config, dst: ChainId, src: ChainId, dst_client: ClientId,
              src_client: ClientId) -> ConnectionId:
    cmd = TxConnInit(dst_chain_id=dst,
                     src_chain_id=src,
...
def conn_try(c: Config, dst: ChainId, src: ChainId, dst_client: ClientId,
             src_client: ClientId, src_conn: ConnectionId) -> ConnectionId:
    cmd = TxConnTry(dst_chain_id=dst,
                    src_chain_id=src,
                    dst_client_id=dst_client,
                    src_client_id=src_client,
...
def conn_ack(c: Config, dst: ChainId, src: ChainId, dst_client: ClientId,
             src_client: ClientId, dst_conn: ConnectionId,
             src_conn: ConnectionId) -> ConnectionId:
    cmd = TxConnAck(dst_chain_id=dst,
                    src_chain_id=src,
                    dst_client_id=dst_client,
                    src_client_id=src_client,
...
def conn_confirm(c: Config, dst: ChainId, src: ChainId, dst_client: ClientId,
                 src_client: ClientId, dst_conn: ConnectionId,
                 src_conn: ConnectionId) -> ConnectionId:
    cmd = TxConnConfirm(dst_chain_id=dst,
                        src_chain_id=src,
                        dst_client_id=dst_client,
                        src_client_id=src_client,
                        dst_conn_id=dst_conn,
...
def query_connection_end(c: Config, chain_id: ChainId,
                         conn_id: ConnectionId) -> ConnectionEnd:
...
def verify_state(c: Config, ibc1: ChainId, ibc0: ChainId,
    ...
    if strategy == 'all':
        ...
        for i in range(20):
            ...
            assert (ibc0_conn_end.state == 'Open'), (ibc0_conn_end,
                                                     "state is not Open")
            assert (ibc1_conn_end.state == 'Open'), (ibc1_conn_end,
                                                     "state is not Open")
...
def passive_connection_start_then_init(
        c: Config, ibc1: ChainId, ibc0: ChainId, ibc1_client_id: ClientId,
        ibc0_client_id: ClientId) -> ConnectionId:
...
def passive_connection_init_then_start(c: Config, ibc1: ChainId, ibc0: ChainId,
                                       ibc1_client_id: ClientId,
                                       ibc0_client_id: ClientId):
...
def passive_connection_try_then_start(c: Config, ibc1: ChainId, ibc0: ChainId,
                                      ibc1_client_id: ClientId,
                                      ibc0_client_id: ClientId):
    ...
    ibc0_conn_id_b = conn_try(c,
                              dst=ibc0,
                              src=ibc1,
                              dst_client=ibc0_client_id,
                              src_client=ibc1_client_id,
                              src_conn=ibc1_conn_id_a)
